...
- [ ] **M011:** TBD — Next milestone
  ...
  - S05 ✅ Admin Dashboard — `(admin)` route group with ADMIN_USER_IDS env-var access gate (non-admins get 404, not redirect). Admin layout renders operator header with user email + sign-out. `/admin` page fetches 9 Supabase queries in a single Promise.all via supabaseAdmin (bypasses RLS): 6 stat cards (Total Teachers, Stripe Active, Published, Total Bookings, Completed Sessions, Revenue) + 15-item recent activity feed merging teacher signups, new bookings, and completions sorted by timestamp. 9 unit tests (admin-dashboard.test.ts). Full suite: 474 tests, 0 failures.

---
*Last updated: 2026-04-01 after M010 completion — Parent & Admin. Full parent account experience (multi-child management, saved payment cards, real-time teacher-parent messaging), Google SSO fix, read-only admin dashboard. 5 slices, 55 new tests (474 total), 4 new DB migrations, 95 files changed. All 14 M010 requirements validated. Next: M011 TBD.*
