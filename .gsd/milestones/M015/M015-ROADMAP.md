...
| S04 | Wire Rate Limits to Public Endpoints | low | — | ✅ | After this: Burst traffic against /api/waitlist, /api/track-view, /api/verify-email, and the login action returns 429 after hitting the limit. Legitimate requests still work. |
| S05 | End-to-End Booking Flow Test | medium | — | ✅ | After this: `npx playwright test booking-e2e` signs up a test parent, navigates to a seeded test teacher profile, books a session, completes Stripe test card payment, verifies the booking email arrived in a test inbox, cancels the booking, and verifies the cancellation email arrived. All green. |
