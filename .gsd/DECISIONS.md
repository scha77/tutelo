...
| D006 | M007/S01/T01 | architecture | Capacity utility split: pure isAtCapacity() vs async getCapacityStatus() | Two exports: isAtCapacity(count, limit) pure function + getCapacityStatus(supabase, teacherId, limit) async function | Pure function enables unit testing without Supabase mocks while the async wrapper handles the DB query layer. On query error, getCapacityStatus returns safe default (not at capacity) so the booking calendar remains visible — explicit failure mode defined in S01 slice spec. | Yes | agent |
| D007 | M007/S01/T03 | architecture | How to handle unauthenticated waitlist inserts from parents | Use supabaseAdmin (service role) for waitlist inserts in /api/waitlist route | Parents browsing a teacher profile are unauthenticated. RLS allows anon inserts on the waitlist table, but using supabaseAdmin via the API route is simpler and more reliable — it bypasses RLS while keeping the endpoint server-side controlled. The API route itself validates teacherId and email format before insert, providing equivalent safety. | Yes | agent |
