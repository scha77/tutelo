...
plan: 04
subsystem: ui
tags: [react-hook-form, zod, supabase, storage, slugify, onboarding, wizard, server-actions]

# Dependency graph
requires:
  - phase: 01-foundation
    provides: "01-02 Supabase Auth with getClaims() + session middleware"
  - phase: 01-foundation
...
affects:
  - "01-05: public profile page reads teachers + availability rows written here"
  - "02-01: booking form reads teacher slug, timezone, availability rows"
  - "03-01: Stripe Connect reads teacher.user_id, links to slug"

# Tech tracking
tech-stack:
  added: [react-hook-form, @hookform/resolvers, zod, slugify]
  patterns:
    - "Server Actions marked 'use server', auth via getClaims(), return { error? }"
    - "FormProvider wraps wizard, per-step validation via form.trigger(STEP_FIELDS[step])"
    - "Insert-then-update pattern for partial saves (avoids NOT NULL constraint on upsert)"
    - "Auto-slug on first teacher row insert in signIn action"
...
key-files:
  created:
    ...
    - supabase/migrations/0002_teachers_user_id_unique.sql
  modified:
    - src/actions/auth.ts
    - src/app/layout.tsx
...
requirements-completed: [ONBOARD-01, ONBOARD-02, ONBOARD-03, ONBOARD-04, ONBOARD-05, ONBOARD-06, ONBOARD-07]
